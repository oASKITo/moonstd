...
function main()
    ...
    -- Команды
        ...
        sampRegisterChatCommand("ttt", cmd_ttt)name = 'Moon AutoLogin'script_name = 'Moon AutoLogin'
script_prefix = '{068aff}[Moon AutoLogin] '
script_author = 'ASKIT'
script_version = '28.09.2021'
    ...
    if mainWindowState.v then
        ...
        imgui.Begin(script_name..' '..script_version, mainWindowState, imgui.WindowFlags.NoResize)
            ...
            if imgui.Checkbox(u8'Авто-ввод паролей', active) then
                -- sampAddChatMessage(script_prefix..'{ffffff}Авто-ввод паролей '..(active.v and 'включен.' or 'отключен.'), -1)
                cfg.settings.active = script_active.v
                inicfg.save(cfg)
                Process()
            end
            imgui.Spacing()
